...
# --- Logging Setup ---
# LOG_FILE_PATH = os.path.join(os.path.dirname(__file__), "todo_server_activity.log")
# logging.basicConfig(
#     level=logging.DEBUG,
...
def read_tasks() -> list:
    """Reads tasks from the todo file."""
...
def write_tasks(tasks: list) -> None:
    """Writes tasks to the todo file."""
    with open(TODO_FILE_PATH, "w") as file:
        for task in tasks:
            file.write(f"{task}\n")

def add_task(task: str) -> dict[str, Any]:
...
def delete_task(task: str) -> dict[str, Any]:
...
# Wrap file utility functions as ADK FunctionTools
ADK_TODO_TOOLS = {
...
#         tool_declaration = adk_tool_instance._get_declaration()
#         if tool_declaration is None:
#             logging.error(f"Tool declaration for {tool_name} is None.")
...
async def list_mcp_tools() -> list[mcp_types.Tool]:
    ...
    for tool_name, adk_tool_instance in ADK_TODO_TOOLS.items():
        try:
            ...
            # Convert ADK tool directly to MCP tool schema
