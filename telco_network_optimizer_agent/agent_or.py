"""
OpenRouter-specific Agent for Telecom Network Optimization
...
# Create the root agent using OpenRouter with Grok model
try:
    root_agent = UnifiedAIAgent(
        provider="openrouter",
        model=os.getenv("OPENROUTER_MODEL", "grok-2-latest"),
        system_prompt=MERGED_MCP_PROMPT
    )
    print(f"✓ OpenRouter agent created: {root_agent}")
except Exception as e:
    print(f"✗ Failed to create OpenRouter agent: {e}")
    root_agent = None

# Example usage
async def main():
    if root_agent is None:
        print("✗ No agent available. Please check your OPENROUTER_API_KEY.")
        return
    
    print("\n" + "="*60)
    print("Testing OpenRouter Agent (Grok)")
    print("="*60 + "\n")
